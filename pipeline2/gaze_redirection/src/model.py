    ...
    def eval(self):
        ...
        with tf.Session(config=tf_config) as test_sess:
            ...
            with test_sess.graph.as_default():
                ...
                gene_dir = os.path.join(imgs_dir, 'genes_%d' % (self.params.ids))
                os.makedirs(gene_dir)

                try:
                    i = 0 # sequence number to prevent duplicates overwriting each other
                    while True:
                        (keys, real_imgs, fake_imgs,
                         a_r, a_t) = test_sess.run(
                            [self.labels_test, self.x_test_r, x_fake,
                             self.angles_test_r, self.angles_test_g])
                        a_t = a_t * np.array([15, 10])
                        a_r = a_r * np.array([15, 10])
                        delta = angular_error(a_t, a_r)
                        
                        keys = [key.decode("utf-8") for key in keys] # convert literal byte to string
                        idxs = [int(key.split('_')[0]) for key in keys] # particpant number
                        ...
                        for j in range(real_imgs.shape[0]):
                            imwrite(os.path.join(
                                gene_dir,
                                '%04d_%04d_%.2f_%s_H%.4f_V%.4f_%s.jpg' % (
                                    idxs[j], i, delta[j], head_poses[j],
                                    a_t[j][0], a_t[j][1], sides[j])),
                                fake_imgs[j]
                                )
                            i += 1

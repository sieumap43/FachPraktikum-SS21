...
home = '../../'
root = os.path.join(home, "MPIIGaze")
sample_root = os.path.join(home, "MPIIGaze/Evaluation Subset/sample list for eye image")
out_root = "."
scale =True
img_scale = 1
imsize = (int(64*img_scale), int(64*img_scale)) # MY MODIFICATION
...
def ImageProcessing_Person(im_root, sample_list, im_outpath, label_outpath, person):
    ...
    for count, info in enumerate(im_list):
        ...
        # im = cv.imread(im_path, cv2.IMREAD_GRAYSCALE)
        im = cv2.imread(im_path, cv2.IMREAD_COLOR) # MY MODIFICATION
        ...
        # im_eye = cv2.equalizeHist(im_eye) # MY MODIFICATION

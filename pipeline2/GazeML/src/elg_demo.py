    ...
    with tf.Session(config=session_config) as session:
        ...
        def _visualize_output():
            ...
            while True:
                ...
                # print("heatmaps", output['heatmaps'].shape) # DEBUG
                # print("landmarks", output['landmarks'].shape) # DEBUG
                # print("radius", output['radius'].shape)
                # print("frame_index", output['frame_index'].shape)
                # print("eye", output['eye'].shape)
                # print("eye_index", output['eye_index'].shape, output['eye_index'][0].shape, output['eye_index'][1].shape)
                bgr = None
                for j in range(batch_size):
                    ...
                    if can_use_eye:
                        ...
                        print(np.mean(gaze_history, axis=0))
                        ...
                        eh, ew, _ = eye_image_raw.shape  
                        gmap_iris, gmap_eyeball =  gm.from_gaze2d((theta, phi), (eh, ew))

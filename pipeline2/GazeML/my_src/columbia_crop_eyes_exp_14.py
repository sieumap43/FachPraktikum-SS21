...
Subprocess for parallelization in experiment 14
...
# destination h5 file
parser.add_argument('--dst_dir', type=str, default='datasets/columbia_cropped',
                    help='destination h5 file')

# destination h5 file
parser.add_argument('--dst_file', type=str, default='datasets/columbia_cropped',
                    help='destination h5 file')
...
parser.add_argument('--participant', type=str,
                    help='process only one single participant')
...
if __name__ == '__main__':
    ...
    img_paths = glob(os.path.join(src_dir, '%s_*.jpg' % (params.participant)))
    ...
    # destination dir
    dst_dir = os.path.join(root_dir, params.dst_dir)

    # train and test have the same data because k-fold cross-validation is being employed
    converter.write_jpg(dst_dir=dst_dir, img_paths=img_paths)